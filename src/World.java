...
public class World {
    ...
    private static void command(String string) throws IOException {
        if (player == null) {
            player = new Player(100, 0, 0, string, 20, 20);
        ...
        switch (string) {
            case "1": {
                ...
                System.out.println("Торговец опять где-то пропадает!!!");
                command(bufferedReader.readLine());
            }
            break;
            case "2": {
        ...
        battle.fight(player, monster(), new FightCallback() {
            ...
            public void fightWin() {
                System.out.println(String.format("%s победил! Теперь у тебя %d опыта и %d золота, осталось %d здоровья.",
                        player.getName(), player.getExp(), player.getMoney(), player.getHp()));
    ...
    private static Character monster() {
        ...
        if (random % 2 == 0) {
            return new Goblin(50, 10, 15, "Гоблин", 20, 5);
        } else
            return new Skeleton(40, 20, 10, "Скелет", 15, 10);
